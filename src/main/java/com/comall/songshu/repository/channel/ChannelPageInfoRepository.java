...
public interface ChannelPageInfoRepository extends JpaRepository<Author,Long> {
     ...
     * @return
     */
    @Query(value = "SELECT UPPER(install.utm_source) AS advSource ,COALESCE(install.memberCount,0) AS installCount,COALESCE(reg.memberCount,0) AS regCount " +
     ...
     * @return
     */
    @Query(value = "SELECT UPPER(reg.utm_source) AS advSource ,0 AS installCount,COALESCE(reg.memberCount,0) AS regCount " +
        ...
        "            FROM songshu_cs_member mem " +
        "            LEFT JOIN songshu_shence_users u  ON u.second_id = mem.\"id\" " +
        "            WHERE  mem.\"regTime\" BETWEEN ?1 AND ?2 and mem.\"multipleChannelsId\" NOT IN (1,2) " +
        "           )base " +
        "           GROUP BY base.utm_source " +
        "     )reg " +
        "ORDER BY reg.memberCount DESC", nativeQuery = true)
    ...
    @Query(value = "SELECT UPPER(reg.utm_source) AS advSource ,0 AS installCount,COALESCE(reg.memberCount,0) AS regCount " +
        ...
        "            FROM songshu_cs_member mem " +
        "            LEFT JOIN songshu_shence_users u  ON u.second_id = mem.\"id\" " +
        "            WHERE  mem.\"regTime\" BETWEEN ?1 AND ?2 AND mem.\"multipleChannelsId\" = ?3 " +
        "           )base " +
        "           GROUP BY base.utm_source " +
        "     )reg " +
        "ORDER BY reg.memberCount DESC", nativeQuery = true)
