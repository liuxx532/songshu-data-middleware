...
public interface ChannelGrossMarginRateRepository extends JpaRepository<Author,Long> {
    ...
    // ON oo."MemberId" = tsource.memberId WHERE cop."PaymentStatus" = 1 AND cpr."PaymentModeType" = 2
    // AND cpr."PaidTime" BETWEEN '2016-01-01 00:00:00' AND '2017-03-01 00:00:00' AND oo."OrderStatus" NOT IN (6, 7)
    // AND oo."orderType" IN (0, 1) AND oo."Channel" IN (0, 1, 2, 3, 5) AND tsource.utm_source = 'YINGYONGBAO') coo
    // ON co."Id" = coo."Id" INNER JOIN songshu_cs_order_item coi ON co."Id" = coi."OrderId") grossmargin
    ...
    @Query(value = "SELECT(grossmargin.AfterFoldingPrice - grossmargin.referCost) / grossmargin.AfterFoldingPrice AS goodsGrossMargin " +
        ...
        "AND oo.\"orderType\" IN (0, 1) AND oo.\"Channel\" IN (0, 1, 2, 3, 5) AND tsource.utm_source = ?3 ) coo " +
        "ON co.\"Id\" = coo.\"Id\" INNER JOIN songshu_cs_order_item coi ON co.\"Id\" = coi.\"OrderId\") grossmargin", nativeQuery = true)
    ...
    @Query(value = "SELECT(grossmargin.AfterFoldingPrice - grossmargin.referCost) / grossmargin.AfterFoldingPrice AS goodsGrossMargin " +
        ...
        "WHERE cop.\"PaymentStatus\" = 1 AND cpr.\"PaymentModeType\" = 2 AND cpr.\"PaidTime\" " +
        "BETWEEN ?1 AND ?2 AND oo.\"OrderStatus\" NOT IN (6, 7) " +
        "AND oo.\"orderType\" IN (0, 1) AND oo.\"Channel\" = ?3 AND tsource.utm_source = ?4 ) coo " +
        "ON co.\"Id\" = coo.\"Id\" INNER JOIN songshu_cs_order_item coi ON co.\"Id\" = coi.\"OrderId\") grossmargin", nativeQuery = true)
